...
\hbadness=10000
\begin{abstract}
	This paper introduces an open-source and optimization-based integrated AC/DC power flow model designed to analyze complex hybrid power grids, such as the Brazilian Interconnected Power System (BIPS). The BIPS is one of the biggest and most complex power grids that interconnects five regions and has high presence of Flexible AC Transmission System (FACTS) devices, including the Thyristor Controlled Series Capacitor (TCSC) and Static Var Compensator (SVC), and bipolar point-to-point line-commutated converters (LCC). The model is formulated as a nonlinear programming (NLP) problem, combining power balance equations, detailed steady-state LCC and FACTS devices equations. In this implementation, the only explicit inequality constraints are those associated with converter station transformer tap limits and delay/extinction angles. All remaining device and network equations are enforced through equality constraints. To handle possible SVC saturation, a post-solution iteration is run to verify reactive limit violations and update each saturated device to its corresponding limit region. The proposed model was implemented in Python using the Pyomo optimization framework and solved using the Ipopt solver and  validated with the Anarede software, on a 247-bus BIPS equivalent and further scaled through a large-scale 7308-bus BIPS model. Results show consistent operating points with expected numerical differences due to different assumptions and solution strategies, while demonstrating robust convergence for large-scale AC/DC systems with complex technology integration.
...
In an optimization-based power flow, the objective function $f(\mathbf{x})$ can be defined as a constant to find any feasible point.
In this work, to provide a clear convergence direction for the solver, the objective function is defined as the squared sum of all
generator active powers \cite{ref11}:
\begin{equation}
	\min_{\mathbf{x}} \quad f(\mathbf{x}) = \sum_{g \in \mathcal{G}} (p_g)^2
	\label{eq:obj_function}
\end{equation}
subject to:
\begin{align}
	\mathbf{g}(\mathbf{x}) & = 0, \label{eq:equality_cons}      \\
	\mathbf{h}(\mathbf{x}) & \leq 0. \label{eq:inequality_cons}
\end{align}
Where $\mathbf{x}$ is the vector of state variables, $\mathcal{G}$ is the set of all generators in the system, and $p_g$ represents the
active power injection of generator $g$. The equality constraints $\mathbf{g}(\mathbf{x})$ represent the complete set of AC and DC power
balance equations. The inequality constraints $\mathbf{h}(\mathbf{x})$ enforce operational limits, such as tap limits and firing/extinction
angles for converter stations.
% ====================================================================================================================================
...
\end{align}
Using these aggregated boundary flows, the active and reactive power mismatches are enforced at each bus $i \in \mathcal{N}$ by:
\begin{equation}
	\label{eq:power_flow_1}
	\sum_{g\in\mathcal{G}_i} p_g - \sum_{d\in\mathcal{D}_i} p_d - P_i^{\ell} - P_i^{csc} - P_i^{dc} - g_i^{sh}v_i^2 = 0,
\end{equation}
\begin{equation}
	\label{eq:power_flow_2}
	\sum_{g\in\mathcal{G}_i} q_g - \sum_{d\in\mathcal{D}_i} q_d + b_i^{sh}v_i^2 + Q_i^{svc} - Q_i^{\ell} - Q_i^{csc} - Q_i^{dc} = 0.
\end{equation}
...
\begin{figure}[!htbp]
	...
	\begin{minipage}{0.48\columnwidth}
		\centering
		\includegraphics[width=\textwidth]{img/svc_linear_with_q.png}
		{\footnotesize (a) SVC linear with $Q$.}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\columnwidth}
		\centering
		\includegraphics[width=\textwidth]{img/svc_linear_with_i.png}
...
\begin{figure*}[!t]
	\centering
	\includegraphics[width=7in]{img/lcc_converters.png}
...
\begin{figure}[!ht]   % <--- Changed from [H] to [!ht]
	\centering
	\includegraphics[width=\columnwidth]{img/brazil_sys.png}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.95\columnwidth]{img/LENA_CASO_FINAL_EQV2020_DC_Full_bus_voltage_angle_results.pdf} \\[0.3em]
	\includegraphics[width=0.95\columnwidth]{img/LENA_CASO_FINAL_EQV2020_DC_Full_generation_active_reactive.pdf} \\[0.3em]
	\includegraphics[width=0.95\columnwidth]{img/LENA_CASO_FINAL_EQV2020_DC_Full_branch_flow_active_reactive.pdf} \\[0.3em]
	\includegraphics[width=0.95\columnwidth]{img/LENA_CASO_FINAL_EQV2020_DC_Full_csc_svc_results.pdf}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.95\columnwidth]{img/LENA_CASO_FINAL_EQV2020_DC_Full_hvdc_reactive_power.pdf} \\[0.5em]
	\includegraphics[width=0.95\columnwidth]{img/LENA_CASO_FINAL_EQV2020_DC_Full_hvdc_commutation_angle.pdf} \\[0.5em]
	\includegraphics[width=0.95\columnwidth]{img/LENA_CASO_FINAL_EQV2020_DC_Full_hvdc_transformer_tap.pdf}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.95\columnwidth]{img/LEN_A_4_2020_SECO_2023VM_SE_EXP_N_4DC_bus_voltage_angle.pdf} \\[0.5em]
	\includegraphics[width=0.95\columnwidth]{img/LEN_A_4_2020_SECO_2023VM_SE_EXP_N_4DC_generation_scatter.pdf}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.95\columnwidth]{img/LEN_A_4_2020_SECO_2023VM_SE_EXP_N_4DC_branch_flow_scatter.pdf} \\[0.5em]
	\includegraphics[width=0.95\columnwidth]{img/LEN_A_4_2020_SECO_2023VM_SE_EXP_N_4DC_dcsc_dcer_scatter.pdf}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.95\columnwidth]{img/LEN_A_4_2020_SECO_2023VM_SE_EXP_N_DC_Full_hvdc_reactive_power.pdf} \\[0.5em]
	\includegraphics[width=0.95\columnwidth]{img/LEN_A_4_2020_SECO_2023VM_SE_EXP_N_DC_Full_hvdc_commutation_angle.pdf} \\[0.5em]
	\includegraphics[width=0.95\columnwidth]{img/LEN_A_4_2020_SECO_2023VM_SE_EXP_N_DC_Full_hvdc_transformer_tap.pdf}
	\caption{Validation of BIPS model at DC components.}
	\label{fig:bips_model_ac_components}
\end{figure}
In terms of computational performance, the proposed optimization-based approach requires more processing time as the system scale increases
compared to traditional sequential power flow solvers. For the BIPS network, the proposed NLP formulation converges after approximately 80
iterations, requiring around 20 seconds of total execution time. In comparison, the commercial software Anarede solves the same network configuration
in 5 seconds and 10 iterations. This computational tradeoff is expected, as the proposed model solves the full non-linear AC/DC system constraints in
a single optimization problem, whereas sequential tools rely on decoupled approaches where the AC and DC networks are solved separately.
...
DC network constraints, and converter control characteristics. The feasibility of the model was demonstrated using both an equivalent and
the full BIPS models, and validated against the commercial software Anarede. While numerical differences exist between the proposed model and
Anarede due to specific assumptions and component modeling, the results demonstrate that the formulation is a reliable starting point for analyzing
the complexity of the BIPS.
