...
_DEFAULT_CONFIG = {
    ...
    "name_aliases": {
        ...
        "voltage": "V",
        "angle": "A0",
        "active_generation": "Pg",
        "reactive_generation": "Qg",
        "maximum_reactive_generation": "Qgm",
        "minimum_reactive_generation": "Qgn",
        "active_load": "Pl",
        "reactive_load": "Ql",
        "shunt_susceptance": "Bsh",
        "maximum_voltage": "Vmx",
        "minimum_voltage": "Vmn",
        "from_bus": "I",
        "to_bus": "J",
...
def _sections(system: Any) -> dict[str, list[dict[str, Any]]]:
    ...
    result["DBAR"] = [
        {
            ...
            "name": bus.name,
            "type": _bus_type(bus.bustype),
            "area": int(getattr(bus.area, "area_number", 0) or 0),
            "voltage": _magnitude(bus.initial_voltage, 1.0),
            "angle": _magnitude(bus.angle),
            "active_generation": 0.0,
            "reactive_generation": 0.0,
            "active_load": 0.0,
            "reactive_load": 0.0,
            "shunt_susceptance": _magnitude(bus.shunt_susceptance),
            "maximum_voltage": _magnitude(bus.voltage_limits.max, 1.1),
            "minimum_voltage": _magnitude(bus.voltage_limits.min, 0.9),
        }
        for bus in buses
    ]
    for block, section in (
        ...
        ("DGER", "DGER"),
        ("DLIN", "DLIN"),
        ("DLIN_TRANSFORMER", "DLIN_TRANSFORMER"),
        ("DSHUNT", "DSHUNT"),
    ):
...
def _component_row(component: Any, block: str) -> dict[str, Any]:
    ...
    if block == "DCAI":
        return {
            "bus": int(float(values[0])),
            "identifier": values[1],
            "status": int(float(values[2])),
            "active_power": float(values[3]),
            "reactive_power": float(values[4]),
        }
    if block == "DGER":
        return {
            "bus": int(float(values[0])),
            "identifier": values[1],
            "active_generation": float(values[2]),
            "reactive_generation": float(values[3]),
            "maximum_reactive_generation": float(values[4]),
            "minimum_reactive_generation": float(values[5]),
            "voltage": float(values[6]),
            "controlled_bus": int(float(values[7])),
            "status": int(float(values[12])),
        }
    if block == "DLIN":
        return {
            ...
            "circuit": values[2],
            "resistance": float(values[3]),
            "reactance": float(values[4]),
            "charging": float(values[5]),
            "rating": float(values[6]),
        }
    if block == "DSHUNT":
        return {
            "bus": int(float(values[0])),
            "control_mode": int(float(values[1])),
            "maximum_voltage": float(values[2]),
            "minimum_voltage": float(values[3]),
            "controlled_bus": int(float(values[4])),
            "reactive_power": float(values[5]),
            "status": int(float(values[6])),
        }
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Convert an ANAREDE NTW file to native C++ DAT format."
    )
